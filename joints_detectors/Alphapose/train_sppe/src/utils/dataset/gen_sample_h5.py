...
def cvt_output2coco(images_dir, output):
    '''
    ...
    anno_info = []
    pose_id = 0
    for file_name in glob.glob(os.path.join(images_dir, '[0-9]*[0-9].jpg')):
        image_id = int(file_name.split('/')[-1].split('.')[0])
        images_info.append({'file_name':file_name, 'id':image_id})

        data_npz = file_name.replace('.jpg', '_hms_data.npz')
        pose_list = np.load(data_npz, allow_pickle=True)['result']['result']
        for pose in pose_list:
            kps = pose['keypoints']
            sco = pose['kp_score']
            coco_kps = []
            for i in range(kps.shape[0]):
                coco_kps.append(kps[i][0])
                coco_kps.append(kps[i][1])
                coco_kps.append(sco[i])
            anno_info.append({'keypoints':coco_kps, 'image_id':image_id, 'id':pose_id})
            pose_id += 1
    
    with open(output, 'w') as f:
        f.write(json.dump({'images':images_info, 'annotations':anno_info}))
...
if __name__ == '__main__':
    ...
    if args.mode == 'h36m2h5':
        ...
        cvt_output2coco(args.dir, args.output)
